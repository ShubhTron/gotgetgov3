...
export function DiscoverPage() {
  ...
  const fetchPlayers = useCallback(async () => {
    ...
    // Get current user sport profile
    const mySpRes = await supabase.from('user_sport_profiles')
      .select('sport, self_assessed_level, availability')
      .eq('user_id', user.id);
    const mySpProfiles: any[] = mySpRes.data ?? [];
