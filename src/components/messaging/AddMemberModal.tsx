...
import { addGroupMember } from '@/lib/messaging';
...
export function AddMemberModal({
  ...
  const loadConnections = async () => {
    ...
    for (const row of (outRes.data || [])) {
      ...
      if (p && !seen.has(p.id) && !memberSet.has(p.id)) {
        seen.add(p.id);
  ...
  const atLimit = currentMemberCount >= 50;
            ...
            <div className="max-h-64 overflow-y-auto space-y-1">
              ...
              {!loading && filtered.length === 0 && (
                <p className="text-sm text-center py-6" style={{ color: 'var(--color-t3)' }}>
                  {connections.length === 0 ? 'All your connections are already in this group' : 'No results'}
                </p>
              )}
